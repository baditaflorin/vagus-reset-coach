...
export function SessionHistory({ sessions, analytics, onClear, onExport }: SessionHistoryProps) {
    ...
    <section className="panel">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex gap-2">
          <button className="icon-button" type="button" onClick={onExport} aria-label="Export sessions">
            <Download size={18} />
          </button>
          <button className="icon-button" type="button" onClick={onClear} aria-label="Clear sessions">
            <Trash2 size={18} />
          </button>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">
        ...
        <MiniStat label="Average" value={analytics.averageCoherence.toString()} />
        ...
        {sessions.length === 0 ? (
          ...
          sessions.slice(0, 5).map((session) => (
            <article className="history-row" key={session.id}>
              <div>
                <p className="font-medium text-stone-950">{formatDateTime(session.startedAt)}</p>
                <p className="text-sm text-stone-600">
                  {session.durationSec}s · RMSSD {session.rmssdMs ?? 'n/a'} ms · quality{' '}
...
function Sparkline({ points }: { points: number[] }) {
  ...
  return (
    <svg className="mt-4 h-24 w-full" viewBox={`0 0 ${width} ${height}`} aria-label="Coherence trend">
      <path d={path} fill="none" stroke="#0f766e" strokeLinecap="round" strokeWidth="5" />
    </svg>
  )
}
