...
export function BreathVisualizer({ state, settings, remainingLabel, running }: BreathVisualizerProps) {
  ...
  return (
    <div className="breath-stage" aria-live="polite">
      <svg className="breath-ring" viewBox="0 0 320 320" role="img" aria-label="Breath pacer">
        ...
        />
        <circle className={`breath-core breath-core-${state.phase}`} cx="160" cy="160" r={radius} />
      </svg>
      <div className="breath-copy">
        ...
        <p>{running ? state.instruction : 'Start a reset and follow the expanding guide.'}</p>
