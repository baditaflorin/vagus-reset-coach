...
export function recommendBreathSettings(metrics: PulseMetrics | null): BreathSettings {
...
export function getBreathState(elapsedMs: number, settings: BreathSettings): BreathState {
  const segments = [
    segment('inhale', settings.inhaleSec, 'Inhale', 'Let the breath widen slowly'),
    segment('hold-top', settings.holdTopSec, 'Hold', 'Rest at the top'),
    segment('exhale', settings.exhaleSec, 'Exhale', 'Soften the jaw and lengthen out'),
    segment('hold-bottom', settings.holdBottomSec, 'Pause', 'Wait for the next breath'),
  ...
  for (const entry of segments) {
    ...
    if (cycleElapsedSec <= next) {
      const phaseProgress = clamp((cycleElapsedSec - cursor) / entry.durationSec, 0, 1)
...
function segment(phase: BreathPhase, durationSec: number, label: string, instruction: string) {
