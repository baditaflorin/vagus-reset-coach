...
export async function summarizeSessionsWithDuckDb(sessions: SessionRecord[]): Promise<AnalyticsSummary> {
  ...
  try {
    ...
    try {
      await connection.insertJSONFromPath('sessions.json', { name: 'sessions' })
      ...
      return {
        ...
        averageRmssdMs: row.average_rmssd_ms === null ? null : Math.round(row.average_rmssd_ms),
