...
export function estimatePulseMetrics(samples: SignalSample[]): PulseMetrics {
  ...
  const analysisSamples = samples.filter(
    (sample) => sample.timeMs >= latest - ANALYSIS_WINDOW_MS,
  );
  ...
  const inverted = detectPeaks(
    processed.map((point) => ({ ...point, value: -point.value })),
  );
  const selected =
    scorePeaks(forward) >= scorePeaks(inverted) ? forward : inverted;
...
export function calculateRmssd(intervalsMs: number[]): number | null {
  ...
  const diffs = intervalsMs
    .slice(1)
    .map((interval, index) => interval - intervalsMs[index]);
...
function preprocess(samples: SignalSample[]): ProcessedPoint[] {
  ...
  return detrended.map((point, index) => ({
    timeMs: point.timeMs,
    value:
      (smoothed[index] - mean + (point.value - smoothed[index]) * 0.25) /
      deviation,
  }));
}
...
function detectPeaks(points: ProcessedPoint[]) {
  ...
  for (let index = 2; index < points.length - 2; index += 1) {
    ...
    const isLocalMax =
      current.value > prev.value && current.value >= next.value;
...
function calculateQuality(
  ...
  const fps =
    ...
    Math.max(
      1,
      (samples[samples.length - 1].timeMs - samples[0].timeMs) / 1_000,
    );
  const cadence = clamp((fps - 8) / 14, 0, 1);
  const intervalMean = average(intervalsMs);
  const intervalRegularity = clamp(
    1 - stddev(intervalsMs, intervalMean) / intervalMean,
    0,
    1,
  );
  const amplitude = clamp(
    stddev(
      processed.map((point) => point.value),
      0,
    ) / 0.8,
    0,
    1,
